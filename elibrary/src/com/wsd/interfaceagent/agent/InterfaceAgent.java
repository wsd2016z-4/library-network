package com.wsd.interfaceagent.agent;

import java.util.ArrayList;
import java.util.List;

import com.wsd.interfaceagent.behaviours.AwaitClientServer;
...
public class InterfaceAgent extends Agent{
	...
	private void findConnectedWarehouse() {
		...
		if (elems.length > 2) {
			...
			System.out.println("Szukam warehouse-agent-" + number + "@192.168.43.48:1111/JADE");
			template.addServices(serviceDescription);
			try {
				DFAgentDescription[] result = DFService.search(this, template);
				System.out.println("Po szukaniu:");
				if (result.length > 0) {
					connectedWarehouse = result[0].getName();
					System.out.println("Znalaz³em: " + result[0].getName());
				}
			}
			catch (FIPAException fe) {
				fe.printStackTrace();
